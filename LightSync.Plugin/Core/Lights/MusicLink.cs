...
namespace Ambilight.Lights
{
    ...
    class MusicLink : IDisposable
    {
        ...
        bool TryOpen()
        {
            ...
            // Deliberately Task-based (not the classic Begin/EndConnect APM pair): on modern .NET that pair is
            // itself implemented on top of this same Task machinery, and throwing on our own 3 s timeout without
            // ever calling EndConnect left the underlying Task's eventual fault (the connect attempt really does
            // fail a few seconds later) unobserved - it got rethrown out of nowhere by the GC finalizer thread
            // once collected. ContinueWith on the timeout path below guarantees it is always observed instead.
            var connectTask = _control.ConnectAsync(_ip, _controlPort);
            try
            {
                if (!connectTask.Wait(3000))
                {
                    connectTask.ContinueWith(t => { _ = t.Exception; }, TaskContinuationOptions.OnlyOnFaulted);
                    throw new TimeoutException("bulb did not accept the TCP connection");
                }
            }
            catch (AggregateException ex)
            {
                // Wait() wraps a faulted task's exception in an AggregateException; unwrapped here so callers
                // (which log e.Message) still get the actual "connection refused"/etc. reason, not a generic one.

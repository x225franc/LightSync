        ...
        private bool RetrieveFrame()
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    // Only the netstandard build of SharpDX.DXGI is resolved into this net9.0 project, and that
                    // build only exposes the raw Try* form (the throwing AcquireNextFrame is a net45-only
                    // extension method) - so the result is turned back into a SharpDXException by hand, which
                    // keeps every catch (SharpDXException) below working unchanged.
                    var acquireResult = _outputDuplication.TryAcquireNextFrame(500, out _, out desktopResource);
                    if (acquireResult.Failure)
                        throw new SharpDXException(acquireResult);

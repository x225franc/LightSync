...
namespace Ambilight.DesktopDuplication
{
    internal class DesktopDuplicatorReader : IDesktopDuplicatorReader
    {
        ...
        /// <summary>Cancels the capture thread and lets it dispose the DXGI duplicator on its way out (only one
        /// app can hold an active desktop duplication session, so this must actually finish before anything else
        /// tries to open a new one - e.g. a plugin restart). Safe to call more than once.</summary>
        public void Stop()
        {
            if (IsRunning)
                RefreshCapturingState();

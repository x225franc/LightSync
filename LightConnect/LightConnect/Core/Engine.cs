...
namespace LightConnect.Core
{
    ...
    public class Engine : IDisposable
    {
        ...
        void TickYeelight(BulbState b, int group, DateTime now)
        {
            ...
            if (link == null || !link.IsAlive)
            {
                if (link != null) { Log.Info("Lost the connection to " + b.Ip); DropLink(b); }
            ...
            if (dark)
            {
                ...
                else if ((now - b.LastSentAt).TotalMilliseconds >= 1000.0 / YeelightFps)
                {
            ...
            if ((key != b.LastSentRgb && (now - b.LastSentAt).TotalMilliseconds >= 1000.0 / YeelightFps) || now - b.LastSentAt > KeepAlive)
            {
        ...
        void ConnectYeelight(BulbState b)
        {
            ...
            b.Status = BulbStatus.Unreachable;
            b.Failures++;
            b.NextAttempt = DateTime.UtcNow.AddSeconds(Math.Min(30, 3 * b.Failures));
        ...
        void DropLink(BulbState b)
        {
            var link = b.Link;
            b.Link = null;
            if (link != null) { try { link.Dispose(); } catch { } }

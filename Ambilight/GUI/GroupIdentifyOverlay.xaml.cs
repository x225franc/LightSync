...
{
    /// <summary>
    /// A "which zone is where" overlay, like Windows' own monitor Identify: four colored, labeled bands are shown
    /// over the captured screen for a few seconds. They match the four equal left-to-right columns LinkLogic
    /// actually samples for Chroma "Group 1".."Group 4" (see LinkLogic.ApplyImageToGrid) - and the same colors the
    /// existing group color test uses, so the two stay recognizable as the same four zones.
    /// </summary>
